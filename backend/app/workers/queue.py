...
class JobQueue:
    ...
    async def cache_embedding(
        ...
        cache_key = f"emb:{text_hash}"
        await self.async_redis.setex(cache_key, ttl, json.dumps(embedding))
    ...
    async def get_cached_nl_summary(self, chunk_hash: str) -> Optional[str]:
        ...
        cache_key = f"nl:{chunk_hash}"
        return await self.async_redis.get(cache_key)

    async def cache_nl_summary(
        self, chunk_hash: str, summary: str, ttl: int = 86400
    ) -> None:
        ...
        cache_key = f"nl:{chunk_hash}"
        await self.async_redis.setex(cache_key, ttl, summary)
